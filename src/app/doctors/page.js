...
const DoctorsPage = () => {
  ...
  const filteredDoctors = doctors.filter((doctor) => {
    const matchesCategory = selectedCategory ? doctor.specialty === selectedCategory : true;
    const matchesSearch = searchTerm ? doctor.specialty.toLowerCase().includes(searchTerm.toLowerCase()) : true;
      ...
      <div className="flex-1 p-6 mt-10 md:mt-0 md:ml-72">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredDoctors.length > 0 ? (
            ...
            <p className="text-center col-span-full">No doctors available for this criteria.</p>
          )}
        </div>
      </div>
    </div>
